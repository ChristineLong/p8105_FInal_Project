...
ssamatab1 <- read_delim("./dataset/ssamatab1.txt",
	...
	group_by(state, year) %>% 
	summarize(state_unemployment = sum(unemployment) / sum(unemployment * 100 / rate),
						unemployment = sum(unemployment)) %>% 
	filter(str_length(state) == 2) %>% 
	gather(key = "type_variable", value = "value", state_unemployment:unemployment)

write_csv(ssamatab1, path = "./tidy_ssamatab1.csv")
