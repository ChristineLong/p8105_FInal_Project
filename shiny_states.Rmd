...
merged_data <- full_join(x = unempl, y = merged_firearm_mortality, by = "state_year") %>%
	separate(col = state_year, into = c("state", "year"), sep = "_") %>% 
	gather(key = type_variable, value = Statistics, unemployment_rate:law_strength) %>% 
...
cr_law <- lm(data = gun_spread, formula = crude_rate ~ law_strength_2016_only)
cr_unempl <- lm(data = gun_spread, formula = crude_rate ~ unemployment_rate)


renderPlotly({
	gun_spread %>% 
	plot_ly() %>% 
	add_markers(x = ~unemployment_rate, y = ~crude_rate) %>% 
	add_lines(y = fitted(cr_unempl), x = ~unemployment_rate)
...
renderPlotly({
	gun_spread %>% 
	plot_ly() %>% 
	add_markers(x = ~law_strength_2016_only, y = ~crude_rate) %>% 
	add_lines(y = fitted(cr_law), x = ~law_strength_2016_only)
