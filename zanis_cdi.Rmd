...
cdi_file <- list.files("./dataset/", full.names = TRUE)
cdi_raw <- read_csv(cdi_file[6])
cdi <- cdi_raw %>%
	...
	select(YearStart:DataValue, -DataSource, -Response, -DatavalueFootnote, -DataValueFootnoteSymbol, -YearEnd) %>%
	select(-Topic, -DataValueUnit, -DataValueType) %>% 
	spread(key = Question, value = DataValue)

cdi %>% View	
colnames(cdi) <- c("year", "state", "state_name", "smoking", "disability_65", "self_rated_health", "drinking", "leisure_phys_act", "overweight", "poverty", "diabetes", "sleep", "mental_health")
